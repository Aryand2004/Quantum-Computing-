namespace Quantum.QB4
{
	...
	operation QB4Run () : Unit
    {
		...
		using (qubits = Qubit[4])
		{
			for (test in 1..10000)
			{
				//Fractional constant when qubits measured 
				Set (Zero, qubits[0]);
				Set (Zero, qubits[1]);
				Set (Zero, qubits[2]);
				Set (Zero, qubits[3]);
				...
				//Demo of Hadamard - superposed state 
				//Set (Zero, qubits[0]);
				//H(qubits[0]);

				//Demo of SWAP - switches states of particular qubits 
				//Set (One, qubits[0]);
				//SWAP (qubits[0],qubits[1]);

				//Demo of Controlled-NOT   ( CNOT )
				//if Qubit 0 is state 1 then NOT Qubit 1, vice versa
				//Set (One, qubits[0]);
				//Set (Zero, qubits[1]);
				...
				//Set (Zero, qubits[0]);
				//Set (Zero, qubits[1]);
				//H(qubits[0]); //puts qubit 0 into superposed state
				//CNOT(qubits[0],qubits[1]); 
				//use superposed state as control on CNOT
				...
				//Set (One, qubits[0]); // Always 1 - Ancilla 
				//Set (One, qubits[1]); // Always 1 - Ancilla 
				//Set (One, qubits[2]); //Input of NOT gate.inverted by NOT
				//CCNOT(qubits[0],qubits[1],qubits[2]);

				//Demo of Toffoli Gate in AND configuration
				Set (One, qubits[0]); // Input 1 of AND
				Set (One, qubits[1]); // Input 2 of AND
				Set (Zero, qubits[2]);// Initialized to 0, result is AND
				CCNOT(qubits[0],qubits[1],qubits[2]);
    ...
    {
		let current = M(q1); //Collapses superposition - qubit 0 or 1 
		if (desired != current)
		{
			//if measured value different from desired value then NOT performed on qubit 
			X(q1);
		}   
    }
